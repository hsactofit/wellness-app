...
class _NutritionLoggingScreenState extends State<NutritionLoggingScreen> {
  ...
  Future<void> _estimate() async {
    ...
    try {
      final result = _analysis == null
          ...
          : await ApiService.instance.updateMealAnalysis(
              _analysis!.id,
              description,
            );
    ...
    return GlassCard(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 14),
          _summaryMetric(
            protein.toStringAsFixed(protein == protein.roundToDouble() ? 0 : 1),
            'protein g',
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              _reportMetric(
                '${(analysis.proteinG ?? 0).toStringAsFixed(1)}g',
                'protein',
                const Color(0xFF43B581),
              ),
              const SizedBox(width: 12),
              _reportMetric(
                '${(analysis.carbsG ?? 0).round()}g',
                'carbs',
                const Color(0xFF4D9DE0),
              ),
              const SizedBox(width: 12),
              _reportMetric(
                '${(analysis.fatsG ?? 0).round()}g',
                'fat',
  ...
  Widget _trendSection(bool isDark, Color foreground) {
    final loggedDays = (_tracker?.series ?? const [])
        .where((day) => (day['meal_count'] as num? ?? 0) > 0)
        .toList();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        GlassCard(
          ...
          child: loggedDays.isEmpty
              ...
              : Column(
                  children: loggedDays
                      .take(7)
                      .map((day) => _trendRow(day, isDark, foreground))
                      .toList(),
                ),
        ),
      ],
      ...
      children: [
        ...
        Expanded(
          child: LinearProgressIndicator(
            value: ((day['calories'] as num? ?? 0).toDouble() / 2500).clamp(
              0,
              1,
            ),
            minHeight: 7,
